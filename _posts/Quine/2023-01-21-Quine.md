...
```
물론 방법은 있습니다! `printf`의 형식문자열로 `CODE`를 사용하고, 인자<sub>(parameter)</sub>에도 `CODE`를 사용해, `char CODE[] = "%s"` 의 `%s`에 CODE를 할당합니다. 그렇게 `CODE` 내부에 `CODE를 삽입했습니다. 실행 결과로 나온 코드를 실행 해볼까요?
```
...
콰인은 맞습니다.[^4]
### 2.4 릴레이 콰인
[**이 콰인**](https://github.com/mame/quine-relay)은 여러가지 언어에서 동작하는 콰인입니다. 예를 들어 C에서 코드를 실행하면 C++으로. C++에서 C#으로 소스코드가 출력되는 형식입니다. 근데 128개의 언어에서 동작합니다.    
![128 릴레이 콰인](https://github.com/mame/quine-relay/raw/master/langs.png)
{: .text-center}    
놀랍지 않나요? 이 128개의 언어 중에는 평범한 언어들(C, JAVA, Python ...)도 있지만, 난해한 언어들(BrainF***, Shakespeare, Whitespace ...)들도 있어 존경을 넘어 경외심이 들기까지 합니다. 이뿐만이 아닙니다.
![128 릴레이 콰인 소스코드](https://github.com/mame/quine-relay/blob/master/thumbnail.png?raw=true)
{: .text-center}    
이와중에 소스코드로 우로보로스 아스키아트를 만들었습니다. 이것이 콰인의 최종 단계인거 같습니다.    
## 3. 마무리
이 세상에는 참 이상하고 신기한 코드들이 많은것 같습니다. 특히 2.4 릴레이 콰인을 보고 나서 이 글을 쓰게 됐다고 해도 과언이 아닐정도로 저에게는 충격적이었습니다. 저도 언젠가는 이런 코드를 작성 할 수 있을까요?    
